<template>
  ...
  <el-table-column
    v-else-if="editable && isVisible && editRender"
    v-bind="bindProps"
  >
    <template v-slot:header="scope">
      <slot
        ...
        >
          <template v-if="compName === 'ElSelect'">
            <el-select
              ...
              :value="getRowIdentity(scope.row,scope.column)"
              @input="setRowIdentity(scope.row,scope.column,$event)"
...
import { getComponentName } from '../../componentsType'

export default {
  name: 'DyEditableColumn',
  props: {
    group: Boolean,
    editRender: Object,
  ...
  computed: {
    renderOpts() {
      const editRender = this.editRender;
    ...
    isVisible() {
      if (this.prop && this.customColumnList && this.customColumnList.length) {
        const column = this.customColumnList.find(item => item.prop === this.prop)
    ...
    bindProps() {
      ...
      let clsName;
      if (this.editable) {
        clsName = this.isReadonly ? 'elx_readonly ' : 'elx_edit ';
      ...
      delete props.group;
      delete props.editRender;
      return props
    },
    editable() {
      return this.$editable.editable
    },
    renderComp({ comps, compName }) {
      // 扩展支持的组件
      return comps.includes(compName) || getComponentName(compName)
    },
    // 渲染组件
    compId({ comps, compName }) {
      return comps.includes(compName) ? compName : getComponentName(compName)
    }
  ...
  methods: {
    ...
    setRowIdentity(row, column, value) {
      XEUtils.set(row.data, column.property, value)
    },
    getRendProps(scope) {
      const size = this.$editable.size
      const props = this.renderOpts.props || this.renderOpts.attrs
      return size ? Object.assign({ size }, props) : props
    },
    getRendEvents({ $index, row, column, store }) {
      let type = 'change';
      const scope = { $index, row: row.data, column, store, $render: this.renderOpts, _row: row }
      ...
      const defEvent = { [type]: evnt => this.$editable.updateStatus(scope) };
    ...
    formatRowIndex(scope) {
      const $index = this.getRowIndex(scope)
    ...
    getSelectLabel({ row, column }) {
      ...
      const props = renderOpts.props || this.renderOpts.attrs || {}
      const labelProp = renderOpts.optionProps.label
      const valueProp = renderOpts.optionProps.value
      const optionsProp = renderOpts.optionGroupProps.options
      const value = this.getRowIdentity(row, column)
      if (!(value === null || value === undefined || value === '')) {
        return (props.multiple ? value : [value]).map(renderOpts.optionGroups ? value => {
          const selectItem = XEUtils.find(renderOpts.optionGroups, group => group[optionsProp].find(item => item[valueProp] === value))
          return selectItem ? selectItem[labelProp] : null
        } : value => {
          const selectItem = XEUtils.find(renderOpts.options, item => item[valueProp] === value)
          return selectItem ? selectItem[labelProp] : null
        }).join(';')
      }
      return null
    ...
    getCascaderLabel({ row, column }) {
      const values = this.getRowIdentity(row, column) || []
      const labels = []
      const props = this.renderOpts.props || this.renderOpts.attrs || {}
    ...
    checkRequired({ column, store }) {
      ...
      const editRules = this.$editable.editRules
      if (property && editRules) {
        const columnRules = XEUtils.get(editRules, property)
        if (columnRules) {
    ...
    checkAllEvent() {
      ...
      const _fullData = this.$editable._fullData
    ...
    checkRowEvent({ row }) {
      const selection = this.handleCheckRow()
    ...
    handleCheckRow() {
      if (this.type === 'selection' && this.scrollLoad) {
        const _fullData = this.$editable._fullData
        const selection = _fullData.filter(item => item.scrollChecked)
        const isIndeterminate = selection.length > 0
...
<style src="../../styles/editable-column.css"></style>
